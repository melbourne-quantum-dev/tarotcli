...
class SpreadLayout:
    ...
    def create_reading(
        self, cards: List[DrawnCard], focus_area: FocusArea, question: str | None = None
    ) -> Reading:
        """
...
SPREADS = {"single": SINGLE_CARD, "three": THREE_CARD, "celtic": CELTIC_CROSS}
...
def get_spread(name: str) -> SpreadLayout:
    ...
    if name not in SPREADS:
        ...
        raise ValueError(f"Unknown spread '{name}'. Available: {available}")
    return SPREADS[name]
